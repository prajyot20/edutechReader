...
public class MuPDFActivity extends Activity implements FilePicker.FilePickerSupport
{
	...
	public static String copySelectionFlag = "Dict";
	public static String copiedWord = "Dict";

	/* The core rendering instance */
	enum TopBarMode {Main, Search, Annot, Delete, More, Accept};
	enum AcceptMode {Highlight, Underline, StrikeOut, Ink, Dictionary,Wiki,Translate};
	...
	public void createUI(Bundle savedInstanceState) {
		...
		mSearchButton.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				searchModeOn();
				lowerButtonsView.setVisibility(View.GONE);
			}
		});
...
//		setButtonEnabled(mMoreButton, !reflow);
	...
	private void showButtons() {
		...
		if (!mButtonsVisible) {
			...
			anim = new TranslateAnimation(0, 0, lowerButtonsView.getHeight(), 0);
			anim.setDuration(200);
			anim.setAnimationListener(new Animation.AnimationListener() {
				public void onAnimationStart(Animation animation) {
				}
				public void onAnimationRepeat(Animation animation) {}
				public void onAnimationEnd(Animation animation) {
					lowerButtonsView.setVisibility(View.VISIBLE);
				}
			});
			lowerButtonsView.startAnimation(anim);
		}
	}
	...
	private void hideButtons() {
		if (mButtonsVisible) {
			...
			//new animation for lower buttons

			anim = new TranslateAnimation(0, 0, 0, lowerButtonsView.getHeight());
			anim.setDuration(200);
			anim.setAnimationListener(new Animation.AnimationListener() {
				public void onAnimationStart(Animation animation) {
				}
				public void onAnimationRepeat(Animation animation) {}
				public void onAnimationEnd(Animation animation) {
					lowerButtonsView.setVisibility(View.INVISIBLE);
				}
			});
			lowerButtonsView.startAnimation(anim);
		}
	...
	private void searchModeOff() {
		if (mTopBarMode == TopBarMode.Search) {
			...
			mDocView.resetupChildren();
			lowerButtonsView.setVisibility(View.VISIBLE);
			cancelOrAcceptVerificationButtonsVisibility();
		}
	}
...
//		mMoreButton = (ImageButton)mButtonsView.findViewById(R.id.moreButton);
		lowerButtonsView = mButtonsView.findViewById(R.id.lowerButtons);
		mTopBarSwitcher.setVisibility(View.INVISIBLE);
		mPageNumberView.setVisibility(View.INVISIBLE);
		mInfoView.setVisibility(View.INVISIBLE);
		mPageSlider.setVisibility(View.INVISIBLE);
	...
	private void cancelOrAcceptVerificationButtonsVisibility() {
		View view = mButtonsView.findViewById(R.id.lowerBar2Annot);
		ImageButton cancelAnnotButton = (ImageButton) mButtonsView.findViewById(R.id.cancelAnnotButton);
		ImageButton acceptLowerButton = (ImageButton) mButtonsView.findViewById(R.id.acceptLowerButton);
		if(mTopBarMode != TopBarMode.Annot) {
			cancelAnnotButton.setVisibility(View.VISIBLE);
			acceptLowerButton.setVisibility(View.VISIBLE);
			view.setVisibility(View.GONE);
		} else {
			cancelAnnotButton.setVisibility(View.GONE);
			acceptLowerButton.setVisibility(View.GONE);
	...
	public void OnWikiButtonClick(View v) {
		copySelectionFlag = "wiki";
		mTopBarMode = TopBarMode.Accept;
		mTopBarSwitcher.setDisplayedChild(mTopBarMode.ordinal());
		mAcceptMode = AcceptMode.Wiki;
		mDocView.setMode(MuPDFReaderView.Mode.Selecting);
		mAnnotTypeText.setText(getString(R.string.copy_text));
		showInfo(getString(R.string.select_text));
		cancelOrAcceptVerificationButtonsVisibility();
	...
	public void OnTranslateButtonClick(View v) {
		copySelectionFlag = "translate";
		mTopBarMode = TopBarMode.Accept;
		mTopBarSwitcher.setDisplayedChild(mTopBarMode.ordinal());
		mAcceptMode = AcceptMode.Translate;
		mDocView.setMode(MuPDFReaderView.Mode.Selecting);
		mAnnotTypeText.setText(getString(R.string.copy_text));
		showInfo(getString(R.string.select_text));
		cancelOrAcceptVerificationButtonsVisibility();
	...
	public void OnAcceptButtonClick(View v) {
		...
		switch (mAcceptMode) {
			...
			case Dictionary:
				...
				mTopBarMode = TopBarMode.More;
				showInfo(success?"Loading Meaning":getString(R.string.no_text_selected));
			...
			case Wiki:
				if (pageView != null)
					success = pageView.copySelection();
				mTopBarMode = TopBarMode.More;
				showInfo(success?"Wiki Search":getString(R.string.no_text_selected));
				break;
			...
			case Translate:
				if (pageView != null)
					success = pageView.copySelection();
				mTopBarMode = TopBarMode.More;
				showInfo(success?"Converting":getString(R.string.no_text_selected));
				break;
